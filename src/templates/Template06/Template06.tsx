...
export const Template06: React.FC<Template06Props> = ({
  ...
  return (
    <div
      className={`relative w-full ${isOpen ? "min-h-screen overflow-y-auto" : "h-full max-h-full overflow-hidden"} bg-[#F7FDF9] text-[#134E4A] font-serif antialiased overflow-x-hidden ${className}`}
    >
      {/* Audio Player Toggle */}
      {data.audio.enabled && (
        <AudioPlayerToggle
