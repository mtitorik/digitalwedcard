...
export const Template16: React.FC<Template16Props> = ({
  ...
  return (
    <div
      className={`relative w-full ${isOpen ? "min-h-screen overflow-y-auto" : "h-full max-h-full overflow-hidden"} bg-[#F0F9FF] text-[#082F49] font-sans antialiased overflow-x-hidden box-border transition-colors duration-500 ${className}`}
      style={{
        ...
        cornerDecorations={
          <>
            ...
            <div className="absolute bottom-3 left-3 z-20 pointer-events-none opacity-75">
              <SeasideWaveCorner flipY className="w-14 h-14 sm:w-20 sm:h-20" />
            </div>
            <div className="absolute bottom-3 right-3 z-20 pointer-events-none opacity-75">
              <SeasideWaveCorner flipX flipY className="w-14 h-14 sm:w-20 sm:h-20" />
            </div>
          </>
        }
      >
